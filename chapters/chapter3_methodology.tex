...
Selecting a methodology for EduVerse was not a routine academic step. The
project is not a single-feature application; it combines interdependent
workflows, multiple roles, web and mobile clients, backend or API behavior,
feature toggles, assessment, live sessions, AI support, and an integrated IDE.
These areas influence one another through shared permissions, shared data, and
shared workflow rules, so the development process needed to support refinement
and integration rather than assume that everything could be fixed correctly in
one pass.

A feature-based iterative methodology was therefore appropriate. EduVerse
required repeated cycles of clarification, implementation, validation, and
integration as the platform matured. Vertical-slice development was equally
suitable because each feature needed to become meaningful across analysis,
design, implementation, access control, and testing, while version control and
pull-request review provided a controlled path for integration.
...
consistency needed repeated, not one-time, validation. The same logic also
suited a platform whose important modules span web and mobile clients, shared
backend or API behavior, configurable features, assessment rules, live-session
flows, AI services, and IDE support.
