...
directly reflected in the implemented platform: the architectural context,
...
EduVerse through the schema overview, the ERD, and the physical schema tables
used to summarize the implemented data model at design level.
...
from \db{database-analysis.md}; where exact SQL types are not restated in this
summary, conservative generic types are used. Keeping these tables in the design
...
\EduSchemaRow{id}{\db{uuid}}{PK; summarized here at design level.}
...
\EduSchemaRow{status}{\db{text}}{Membership-like status field; summarized here with its implemented meaning.}
...
\EduSchemaRow{material_type}{\db{text}}{Chat-media material descriptor; summarized here with its implemented meaning.}
...
\EduSchemaRow{entity_id}{design-level summary}{Polymorphic logical relationship to multiple entity tables.}
...
For readability and design clarity, the database schema is presented at a design level rather than as a verbatim reproduction of every implemented \db{CREATE TABLE} definition. The schema tables therefore focus on the columns and relationships used by the platform, while relationships that are not expressed as explicit foreign keys are documented as logical relationships.
