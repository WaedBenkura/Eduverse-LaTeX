\chapterwithopening{System Design}{chap:system_design}{
  ...
  \chaptercontentsitem{sec:integration_and_security_design}
}
...
This chapter presents the system design perspective of Eduverse. In the
current revision, the chapter is intentionally focused on the database design
because the verified schema, regenerated ERD, and supporting database notes
are sufficiently stable to document in an academic manner. The remaining
design sections are retained to preserve the full chapter structure and will
be expanded in later documentation passes.

\section{Architectural Design Overview}\label{sec:architectural_design_overview}

At a high level, Eduverse combines a web-based learning environment with a
mobile companion scope, a Supabase-backed PostgreSQL database, AWS S3-backed
file references for uploaded academic content, and LiveKit-related session
metadata for synchronous learning. A fuller component-level architectural
description is reserved for a later revision, while the present section
establishes the context for the detailed data design discussed in
Section~\ref{sec:data_and_storage_design}.

\section{Data and Storage Design}\label{sec:data_and_storage_design}

This section documents the current verified database design of Eduverse,
including the schema overview, the ERD, the physical schema tables derived
from the latest analysis notes, and the main uncertainty caused by missing
base-table DDL in part of the checked-in migration history.

\subsection{Database Design Overview}\label{subsec:database_design_overview}
...
\subsection{Entity Relationship Diagram (ERD)}\label{subsec:entity_relationship_diagram}

Figure~\ref{fig:eduverse_database_erd} presents the current Eduverse database
ERD derived from the latest checked-in schema evidence and summarizes the
main organization, class, content, assessment, and operational relationships
within the platform.
...
\subsection{Database Tables}\label{subsec:database_tables}

The following schema tables present the real Eduverse database tables on
normal portrait thesis pages. Column names, primary keys, foreign keys,
logical relationships, enum or status fields, and essential notes are taken
from \db{database-analysis.md}; where exact SQL types are not directly
visible, conservative generic types are used.
...
\subsection{Database Design Notes and Uncertain Relationships}
...
\section{User Interface Design}\label{sec:interface_and_interaction_design}

This section presents the main implemented EduVerse interfaces and
illustrates how the platform supports administrative, instructional, and
student-facing workflows through the web application and the broader
companion-application architecture. The currently provided screenshots
document the implemented web views that coordinate organization management,
classroom activity, assessment, live collaboration, and AI-supported study
workflows.

\subsection{Authentication and Access Interfaces}

The authentication and access interfaces define the entry point to EduVerse
and establish the user context from which subsequent permissions are resolved.
The provided views show account entry, organization and role selection for
multi-context users, and the personal settings controls used after
authentication.
...
\subsection{Organization Administration Interfaces}

The organization administration interfaces document the controls available to
organization administrators for creating workspaces, governing class
structures, managing members, and configuring public access. As shown in the
dashboard views, the administrative workspace contains multiple tabs such as
Classes, Past Terms, Users, Features, Public Link, and Settings, with
representative screenshots included below.

\EduUiFigure
  {figures/screenshots/10_Organization Creation and Initial Feature Availability.png}
  {Organization creation interface showing template selection and initial feature-availability controls.}
...
\subsection{Classroom and Learning Workspace Interfaces}

The classroom and learning workspace interfaces present the day-to-day views
through which teachers and students interact with class resources. Together,
these screenshots show the class landing page, shared materials,
communication channels, and notifications that keep classroom activity
coordinated.
...
\subsection{Assignment and Assessment Interfaces}

The assignment and assessment interfaces illustrate how EduVerse manages
...
\subsection{Live Session and Collaboration Interfaces}
...
integrated within a single teaching workspace.
...
\subsection{AI Learning Support and Coding Lab Interfaces}

The AI learning support and coding-lab interfaces extend the classroom beyond
static content delivery by offering guided assistance and hands-on technical
practice. The provided screenshots demonstrate conversational study support
alongside the embedded programming workspace.
...
% TODO: Insert mobile companion application screenshots when provided.

\section{Integration and Security Design}\label{sec:integration_and_security_design}

The broader integration and security design of Eduverse, including service
boundaries, access-control flows, and environment-level protections, will be
expanded in a later revision of this chapter. For the current database-focused
pass, the most relevant design distinction is already captured in
Section~\ref{subsec:database_design_notes_uncertain}, where enforced foreign
keys are separated from logical relationships when the visible migration
