...
does more than list entities: it clarifies cardinalities, one-to-one control
links, one-to-many instructional flows, and many-to-many relationships resolved
through junction tables.

\paragraph*{Principal Relationship Structure}
\begin{itemize}
  \item \textbf{Organization structure.} One organization can govern many
    classes, invitations, join links, settings, and feature records. The
    companion \db{organization_settings} relationship behaves as a one-to-one
    administrative extension of the main organization record.
  \item \textbf{User and membership structure.} One user can belong to many
    organizations through membership records, and each membership connects a
    user to a specific organization context. A membership may then connect to
    one or more role records, allowing active-role resolution without reducing
    governance to one fixed permission entry.
  \item \textbf{Class participation structure.} One class can have many
    members, while users may participate in many classes. This is a many-to-many
    academic relationship resolved through class-membership tables rather than
    through direct ownership alone.
  \item \textbf{Learning-resource structure.} One class can contain many
    materials, messages, notifications, and live sessions, which keeps content,
    communication, and real-time activity tied to the same governed class
    context.
  \item \textbf{Assessment structure.} One class can contain many assignments
    and many exams. One assignment can receive many submissions, while one exam
    can expand into many questions, attempts, and answers, forming the full
    assessment chain from authoring to participation and review.
  \item \textbf{Feature-governance structure.} Feature definitions connect to
    organizations and classes through organization-level and class-level feature
    settings. These are configuration relationships rather than ordinary content
    records, and they determine which tools remain enabled, disabled, or
    further constrained in each context.
\end{itemize}

\paragraph*{Design Significance}
These relationships matter because they support role-based access control,
organization governance, class visibility, academic workflow continuity,
assessment management, and configurable feature availability within one design
model. They also explain why EduVerse relies on intermediate structures rather
than only on direct foreign keys: users and organizations, users and classes,
memberships and roles, and platform contexts and feature settings all require
many-to-many or context-sensitive resolution. The ERD therefore provides the
clearest design-level view of how EduVerse connects institutional control with
practical teaching and learning activity.
